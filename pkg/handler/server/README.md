...
- v2 server image updates are currently rejected with HTTP 422: the in-place
  rebuild that acted on image changes has been removed, so accepting a new
  image would update the spec with nothing to realize it at runtime. Flavor
  changes remain unsupported and are rejected with HTTP 422.
