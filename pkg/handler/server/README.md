...
- changing a v2 server's `imageId` is a destructive in-place Nova rebuild. It
  ...
  Same-image PUTs (clients echoing current state) remain a no-op and do not
  arm a rebuild attempt.
...
- `v2` reads rewrite the provisioning status the core conversion produces:
  `deriveProvisioningStatus` reports `provisioning` in place of a core-derived
  `provisioned` whenever `Server.RebuildPending()` is true and the attempt is
  not parked. `RebuildPending()` is exactly "`Status.Rebuild` is non-nil" —
  armed-but-not-yet-accepted counts as pending, not only an accepted,
  in-flight rebuild. A parked attempt is excluded from the rewrite even though
  `RebuildPending()` is still true for it: a park is terminal and already
  surfaces through the `Available` condition as `error`, so forcing
  `provisioning` over it would hide a failure behind a status that implies
  the server will eventually settle on its own. uni-compute's instance
  settlement gate is the sole consumer that relies on `provisioned` meaning
  fully settled; it never PUTs an image change to region itself, handling
  drift by delete-and-recreate instead, but it still needs an accurate read
  when a human operator or another client arms a rebuild through this API.
