...
// withRebuildMarker records a rebuild intent for the fixture image with the
// given number of Nova-accepted attempts.
func withRebuildMarker(server *regionv1.Server, acceptedAttempts int32) *regionv1.Server {
...
// Nova-accepted rebuild in flight (marker retained, acceptedAttempts > 0) whose
// Available condition already reads Provisioned is reported as provisioning at
// the v2 API — the rebuild's target image is not yet realized, so the server is
// not settled.
func TestServerGetV2RebuildPendingReportsProvisioning(t *testing.T) {
	...
	resource := withRebuildMarker(withAvailableCondition(testServerV2(srvServerID), corev1alpha1.ConditionReasonProvisioned), 1)
...
func TestServerGetV2RebuildPendingErroredStaysError(t *testing.T) {
	...
	resource := withRebuildMarker(withAvailableCondition(testServerV2(srvServerID), corev1alpha1.ConditionReasonErrored), 1)
...
// TestServerGetV2RebuildIntentNotAcceptedReportsProvisioned verifies that a
// recorded rebuild intent that Nova never accepted (acceptedAttempts == 0)
// leaves a Provisioned status unchanged: nothing destructive is in flight, so
// the server is still settled.
func TestServerGetV2RebuildIntentNotAcceptedReportsProvisioned(t *testing.T) {
	t.Parallel()

	ctrl := gomock.NewController(t)

	resource := withRebuildMarker(withAvailableCondition(testServerV2(srvServerID), corev1alpha1.ConditionReasonProvisioned), 0)
