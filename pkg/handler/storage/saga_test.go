...
// TestValidateAttachments_NetworkNotFound tests that when a network ID is specified in the attachments but does not exist, an InvalidRequest error is returned.
...
// TestValidateAttachments_ProjectMismatch tests that when a network exists but belongs to a different project, an InvalidRequest error is returned.
