...
func (h *Handler) PostApiV1OrganizationsOrganizationIDProjectsProjectIDIdentities(w http.ResponseWriter, r *http.Request, organizationID openapi.OrganizationIDParameter, projectID openapi.ProjectIDParameter) {
	...
	if err != nil {
		err = errors.OAuth2ServerError("unable to get region provider").WithError(err)
		errors.HandleError(w, r, err)

		return
	}

	result, err := identity.New(h.client, h.namespace).Create(r.Context(), organizationID, projectID, request, provider)
	if err != nil {
		errors.HandleError(w, r, err)
		return
	}
