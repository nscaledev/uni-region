...
func convertReservations(in *regionv1.NetworkReservations) *openapi.NetworkReservations {
	...
	out := &openapi.NetworkReservations{
		...
		NonStoragePrefixLength: in.NonStoragePrefixLength,
	}
...
func generateReservations(prefix *net.IPNet, in *openapi.NetworkReservations) (*regionv1.NetworkReservations, error) {
	...
	if in.NonStoragePrefixLength != nil && *in.NonStoragePrefixLength < in.PrefixLength {
		return nil, errors.HTTPUnprocessableContent("the VIP pool prefix length must be greater than or equal to the reservation prefix length").WithValues("reservationPrefix", in.PrefixLength, "vipPrefixLength", *in.NonStoragePrefixLength)
	}
...
func (c *Client) Update(ctx context.Context, networkID string, request *openapi.NetworkV2Update) (*openapi.NetworkV2Read, error) {
	...
	reservations, err := generateReservations(&current.Spec.Prefix.IPNet, request.Spec.Reservations)
	if err != nil {
		return nil, err
	}
