/*
Copyright 2025 the Unikorn Authors.
Copyright 2026 Nscale.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

//nolint:testpackage
package image

import (
	"bytes"
	"compress/bzip2"
	"context"
	"io"
	"net/http"
	"testing"
	"time"

	"github.com/gophercloud/gophercloud/v2"
	"github.com/stretchr/testify/require"
	"go.uber.org/mock/gomock"

	coreopenapi "github.com/unikorn-cloud/core/pkg/openapi"
	"github.com/unikorn-cloud/region/pkg/handler/image/mock"
	"github.com/unikorn-cloud/region/pkg/openapi"
	"github.com/unikorn-cloud/region/pkg/providers/types"

	"k8s.io/utils/ptr"
)

const imageID = "b3796b32-57d3-40cf-b43e-d227c0c5a70b"

func expectedReaderBytes(content []byte) gomock.Matcher {
	return gomock.Cond(func(reader io.Reader) bool {
		actual, err := io.ReadAll(reader)
		if err != nil {
			return false
		}

		return bytes.Equal(actual, content)
	})
}

func expectUploadOfBytes(data []byte) func(*mock.MockProvider) {
	return func(provider *mock.MockProvider) {
		provider.EXPECT().
			UploadImageData(gomock.Any(), imageID, expectedReaderBytes(data)).
			Return(nil)
	}
}

func bytesReader(data []byte) func(*testing.T) io.Reader {
	return func(t *testing.T) io.Reader {
		t.Helper()

		return bytes.NewBuffer(data)
	}
}

func TestUploadImageData(t *testing.T) {
	t.Parallel()

	expectedImage := &openapi.Image{
		Metadata: coreopenapi.StaticResourceMetadata{
			Id:           "b3796b32-57d3-40cf-b43e-d227c0c5a70b",
			Name:         "test-image",
			CreationTime: time.Date(2025, 10, 31, 12, 0, 0, 0, time.UTC),
		},
		Spec: openapi.ImageSpec{
			Gpu: &openapi.ImageGpu{
				Driver: "525.85.05",
				Models: ptr.To([]string{
					"A100",
					"H100",
					"H200",
				}),
				Vendor: openapi.GpuVendorNVIDIA,
			},
			Os: openapi.ImageOS{
				Codename: ptr.To("alpha"),
				Distro:   openapi.OsDistroUbuntu,
				Family:   openapi.OsFamilyDebian,
				Kernel:   openapi.OsKernelLinux,
				Variant:  ptr.To("server"),
				Version:  "0.1.0",
			},
			SizeGiB: 25,
			SoftwareVersions: ptr.To(openapi.SoftwareVersions{
				"kubernetes": "v1.25.6",
			}),
			Virtualization: openapi.ImageVirtualizationVirtualized,
		},
	}

	rawFileContent := make([]byte, 512)
	rawFileContent[510], rawFileContent[511] = 0x55, 0xAA
	fakeRawDiskReader := mock.TarballedReader(mock.Files{"disk.raw": rawFileContent})

	qcow2FileContent := []byte("QFI\xfb")
	fakeQcow2Reader := mock.TarballedReader(mock.Files{"disk.qcow2": qcow2FileContent})

	type TestCase struct {
		Name              string
		ContextMutateFunc func(ctx context.Context) context.Context
		ReaderSetupFunc   func(t *testing.T) io.Reader
		ProviderSetupFunc func(provider *mock.MockProvider)
		DiskFormat        types.ImageDiskFormat
		ContentType       string
		ExpectedError     bool
		ExpectedImage     *openapi.Image
	}

	testCases := []TestCase{
		{
			Name:            "fails to create gzip reader",
			ReaderSetupFunc: mock.StringReader("this is not valid gzip data"),
			ExpectedError:   true,
			ExpectedImage:   nil,
		},
		{
			Name:            "fails to read invalid tar data",
			ReaderSetupFunc: mock.GzippedReader(bytes.NewBuffer(([]byte{1, 2, 3, 4, 5, 6, 7, 8, 9}))),
			ExpectedError:   true,
			ExpectedImage:   nil,
		},
		{
			Name: "fails to parse tar header",
			ReaderSetupFunc: mock.GzippedReader(
				mock.Base64Reader(t, "")),
			ExpectedError: true,
			ExpectedImage: nil,
		},
		{
			Name: "fails to parse oversized tar header",
			ReaderSetupFunc: mock.GzippedReader(
				bzip2.NewReader(mock.Base64Reader(t, "QlpoOTFBWSZTWcRfsUcAAFDfkOiQQAH/xgBASEBmQF5AAEAICCAAhBtRU9NNIPUaNPUeppo0PUCqVD1AaADEZNHiiS+Wdi697kCxx9qIT3GF8mQnSKvg2sFNkHGCdCG6MqN4zcKTytwhtItXKSj3rOWtbdRUbOOwCADGRCIAjQnVWdKV05Sa6fk0smrPvHAIAfxdyRThQkMRfsUc"))),
			ExpectedError: true,
			ExpectedImage: nil,
		},
		{
			Name: "fails to copy data into temporary file after context cancellation",
			ContextMutateFunc: func(ctx context.Context) context.Context {
				ctx, cancel := context.WithCancel(ctx)
				cancel()

				return ctx
			},
			ReaderSetupFunc: mock.TarballedReader(mock.Files{"disk.raw": rawFileContent}),
			ExpectedError:   true,
			ExpectedImage:   nil,
		},
		{
			Name:            "fails to find disk file in tar archive",
			ReaderSetupFunc: mock.TarballedReader(mock.Files{"randomfile.txt": []byte("file contents")}),
			ExpectedError:   true,
			ExpectedImage:   nil,
		},
		{
			Name:            "fails to upload image due to upload conflict",
			ReaderSetupFunc: fakeRawDiskReader,
			ProviderSetupFunc: func(provider *mock.MockProvider) {
				provider.EXPECT().
					UploadImageData(gomock.Any(), imageID, expectedReaderBytes(rawFileContent)).
					Return(gophercloud.ErrUnexpectedResponseCode{Actual: http.StatusConflict})
			},
			ExpectedError: true,
			ExpectedImage: nil,
		},
		{
			Name:            "fails to upload image due to unexpected error",
			ReaderSetupFunc: fakeRawDiskReader,
			ProviderSetupFunc: func(provider *mock.MockProvider) {
				provider.EXPECT().
					UploadImageData(gomock.Any(), imageID, expectedReaderBytes(rawFileContent)).
					Return(gophercloud.ErrUnexpectedResponseCode{Actual: http.StatusInternalServerError})
			},
			ExpectedError: true,
			ExpectedImage: nil,
		},
		{
			Name:              "succeeds in uploading tarballed raw image file",
			ReaderSetupFunc:   fakeRawDiskReader,
			ProviderSetupFunc: expectUploadOfBytes(rawFileContent),
			ExpectedError:     false,
			ExpectedImage:     expectedImage,
		},
		{
			Name:              "succeeds in uploading tarballed qcow2 image file",
			ReaderSetupFunc:   fakeQcow2Reader,
			ProviderSetupFunc: expectUploadOfBytes(qcow2FileContent),
			DiskFormat:        types.ImageDiskFormatQCOW2,
			ExpectedError:     false,
			ExpectedImage:     expectedImage,
		},
		{
			Name:              "succeeds in uploading .raw",
			ReaderSetupFunc:   bytesReader(rawFileContent),
			ContentType:       "application/octet-stream",
			ProviderSetupFunc: expectUploadOfBytes(rawFileContent),
			DiskFormat:        types.ImageDiskFormatRaw,
			ExpectedImage:     expectedImage,
		},
		{
			Name:              "succeeds in uploading .qcow2",
			ReaderSetupFunc:   bytesReader(qcow2FileContent),
			ContentType:       "application/octet-stream",
			ProviderSetupFunc: expectUploadOfBytes(qcow2FileContent),
			DiskFormat:        types.ImageDiskFormatQCOW2,
			ExpectedImage:     expectedImage,
		},
	}

	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			t.Parallel()

			mockController := gomock.NewController(t)
			defer mockController.Finish()

			mockProvider := mock.NewMockProvider(mockController)
			if fn := testCase.ProviderSetupFunc; fn != nil {
				fn(mockProvider)
			}

			ctx := t.Context()

			if fn := testCase.ContextMutateFunc; fn != nil {
				ctx = fn(ctx)
			}

			reader := testCase.ReaderSetupFunc(t)

			diskFormat := types.ImageDiskFormatRaw
			if f := testCase.DiskFormat; f != "" {
				diskFormat = f
			}

			contentType := "application/tar+gzip"
			if ct := testCase.ContentType; ct != "" {
				contentType = ct
			}

			upload := uploadToProvider(imageID, mockProvider)

			err := dispatchUpload(ctx, contentType, diskFormat, reader, upload)
			require.Equal(t, testCase.ExpectedError, err != nil, "got error %s", err)
		})
	}
}
