...
import (
	...
	filestorage "github.com/unikorn-cloud/region/pkg/provisioners/managers/file-storage"
...
func (*Factory) Reconciler(options *options.Options, controllerOptions coremanager.ControllerOptions, manager manager.Manager) reconcile.Reconciler {
	return coremanager.NewReconciler(options, controllerOptions, manager, filestorage.New)
}

// RegisterWatches adds any watches that would trigger a reconcile.
func (*Factory) RegisterWatches(manager manager.Manager, controller controller.Controller) error {
	// Any changes to the file storage spec, trigger a reconcile.
	if err := controller.Watch(source.Kind(manager.GetCache(), &unikornv1.FileStorage{}, &handler.TypedEnqueueRequestForObject[*unikornv1.FileStorage]{}, &predicate.TypedGenerationChangedPredicate[*unikornv1.FileStorage]{})); err != nil {
		return err
	}
