...
It is a latency optimisation, not a liveness dependency. `ErrYield` requeues after
`DefaultYieldTimeout`, so an in-flight rebuild is re-read on a timer whether or not
this arm ever fires; the arm only shortens the wait.
