...
import (
	...
	"github.com/unikorn-cloud/region/pkg/provisioners/managers/imagemonitortask"
...
func (*Factory) Options() coremanager.ControllerOptions {
	return &imagemonitortask.Options{}
}
...
func (*Factory) Reconciler(options *options.Options, controllerOptions coremanager.ControllerOptions, manager manager.Manager) reconcile.Reconciler {
	return coremanager.NewReconciler(options, controllerOptions, manager, imagemonitortask.New)
}

// RegisterWatches adds any watches that would trigger a reconcile.
func (*Factory) RegisterWatches(manager manager.Manager, controller controller.Controller) error {
	// Any changes to the image monitor task spec, trigger a reconcile.
	if err := controller.Watch(source.Kind(manager.GetCache(), &unikornv1.ImageMonitorTask{}, &handler.TypedEnqueueRequestForObject[*unikornv1.ImageMonitorTask]{}, &predicate.TypedGenerationChangedPredicate[*unikornv1.ImageMonitorTask]{})); err != nil {
		return err
	}
