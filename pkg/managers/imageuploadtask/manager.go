...
import (
	...
	"github.com/unikorn-cloud/region/pkg/provisioners/managers/imageuploadtask"
...
func (*Factory) Options() coremanager.ControllerOptions {
	return &imageuploadtask.Options{}
}
...
func (*Factory) Reconciler(options *options.Options, controllerOptions coremanager.ControllerOptions, manager manager.Manager) reconcile.Reconciler {
	return coremanager.NewReconciler(options, controllerOptions, manager, imageuploadtask.New)
}

// RegisterWatches adds any watches that would trigger a reconcile.
func (*Factory) RegisterWatches(manager manager.Manager, controller controller.Controller) error {
	// Any changes to the image upload task spec, trigger a reconcile.
	if err := controller.Watch(source.Kind(manager.GetCache(), &unikornv1.ImageUploadTask{}, &handler.TypedEnqueueRequestForObject[*unikornv1.ImageUploadTask]{}, &predicate.TypedGenerationChangedPredicate[*unikornv1.ImageUploadTask]{})); err != nil {
		return err
	}
