...
// TestCheckFailedIdentityReadSparesOtherIdentities is the best-effort
...
func TestCheckFailedIdentityReadSparesOtherIdentities(t *testing.T) {
	...
	require.Contains(t, *harness.sink.errors, "failed to observe identity servers, skipping")
}
...
func TestCheckMissingIdentitySparesOtherIdentities(t *testing.T) {
	...
	require.Equal(t, unikornv1.ActiveConditionReasonBuilding, harness.phase(t, "server-a1"))
	require.Equal(t, unikornv1.ActiveConditionReasonRunning, harness.phase(t, "server-b1"))
	require.Contains(t, *harness.sink.errors, "failed to get identity, skipping servers")
}
...
func TestCheckFailedObserveSparesOtherServers(t *testing.T) {
	...
	require.Contains(t, *harness.sink.errors, "failed to observe server, skipping")
}
...
// TestCheckCancelledContextAbortsTheCycle pins that the cycle's own context
// ending is the one failure that is not best effort.
func TestCheckCancelledContextAbortsTheCycle(t *testing.T) {
	...
	require.Zero(t, harness.reads.Load(), "a cancelled cycle must not start a read")
}

// TestCheckStateGaugeCountsEveryBucket pins the aggregation across buckets: a
// merge that loses one loses those servers from the gauge entirely, which is a
// silent under-count rather than an error.
...
// TestCheckLogsAbsenceWhenNothingWasWritten pins that a server with no provider
// instance is reported even on a cycle that writes nothing. Those servers are
// out of the state gauge, so this log is the only recurring signal that one the
// reconciler cannot recreate is still missing, and gating it on the patch
// silenced it from the second cycle onwards.
func TestCheckLogsAbsenceWhenNothingWasWritten(t *testing.T) {
	t.Parallel()

	const absent = "server not found in provider"

	// Already recorded absent, so the projection matches and nothing is written.
	...
	require.NoError(t, harness.err)
	require.Equal(t, settled.ResourceVersion, harness.resourceVersion(t, "server-gone"),
		"an unchanged observation must not be written")
	require.Len(t, harness.sink.entriesWithMsg(absent), 1,
		"the absence must be reported even when nothing was written")
}
