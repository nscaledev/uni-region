...
- reads and writes no rebuild-marker state (`Status.Rebuild`); the rebuild
  feature is excised pending re-implementation against
...
- The `Active` condition is a live readiness signal once provisioning status
  ...
  healthy operation staleness is bounded by one poll period.
