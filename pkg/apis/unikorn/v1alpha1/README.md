...
- `Server.Status.Observed` is the partition that lets the two status writers stop
  ...
  never observed. `Error` is live state and does clear on an authoritative
  non-error read, which is safe only because an unreachable provider aborts the
  poll without writing at all — connectivity loss can never be mistaken for a
  recovery.
