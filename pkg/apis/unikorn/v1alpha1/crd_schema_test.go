...
func TestVolumeClaimSchema(t *testing.T) {
	...
	pendingServerGeneration := requireSchemaProperty(t, claim, "pendingServerGeneration")
	require.Equal(t, "integer", pendingServerGeneration.Type)
	require.NotNil(t, pendingServerGeneration.Minimum)
	require.InDelta(t, 1, *pendingServerGeneration.Minimum, 0)
	require.NotContains(t, claim.Required, "pendingServerGeneration")
