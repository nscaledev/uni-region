...
// +kubebuilder:resource:scope=Namespaced,categories=unikorn
// +kubebuilder:subresource:status
// +kubebuilder:validation:XValidation:rule="!oldSelf.hasValue() || !has(oldSelf.value().status) || !has(oldSelf.value().status.backendObservedAt) || (has(self.status) && has(self.status.backendObservedAt) && self.status.backendObservedAt == oldSelf.value().status.backendObservedAt)",message="backendObservedAt is write-once",optionalOldSelf=true
// +kubebuilder:printcolumn:name="display name",type="string",JSONPath=".metadata.labels['unikorn-cloud\\.org/name']"
...
type FileStorageSnapshotStatus struct {
	...
	// BackendObservedAt records the first definite matching backend observation.
	// It is internal, write-once lifecycle state and is never publicly projected.
	BackendObservedAt *metav1.Time `json:"backendObservedAt,omitempty"`
