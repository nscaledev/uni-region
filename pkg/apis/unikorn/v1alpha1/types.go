...
type OpenstackVolumeClassesSpec struct {
	...
	// but Region uses the domain term VolumeClass for this inventory.
	// +listType=map
	// +listMapKey=id
...
type VolumeClassMetadata struct {
	...
	// Replicated indicates that volumes provisioned from this class are replicated
	// by the provider.
	Replicated bool `json:"replicated,omitempty"`
}

type VolumeClassPerformanceSpec struct {
	// IOPS describes the advertised input/output operations per second.
	// +kubebuilder:validation:Minimum=1
	IOPS *int `json:"iops,omitempty"`
	// Throughput describes advertised throughput in mebibytes per second.
	// +kubebuilder:validation:Minimum=1
	Throughput *int `json:"throughputMiBps,omitempty"`
}
