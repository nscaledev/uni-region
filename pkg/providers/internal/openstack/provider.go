...
func (p *Provider) imageDataSource(image *images.Image) types.ImageDataSource {
	temp, _ := image.Properties[dataSourceLabel].(string)

	var dataSource types.ImageDataSource

	switch temp {
	case string(types.ImageDataSourceFile), string(types.ImageDataSourceURL):
...
func (p *Provider) GetImage(ctx context.Context, organizationID, imageID string) (*types.Image, error) {
	...
	if err != nil {
		if errors.Is(err, types.ErrResourceNotFound) {
			return nil, fmt.Errorf("image %w", types.ErrResourceNotFound) // elide any more detail than this
		}
