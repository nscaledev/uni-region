...
- An existing server's image drift against spec is not acted on: the in-place
  rebuild is excised (the write-ahead marker, the reconciler pass, and the Nova
  rebuild call) and the API rejects image changes until the re-implementation
  lands, per
  [`2026-07-31-server-rebuild-protocol-design.md`](../../../../docs/superpowers/specs/2026-07-31-server-rebuild-protocol-design.md).
