...
- Baremetal server progress uses Ironic as an additional provider truth source
  ...
  empty fault, and the fault is the operator detail this provider writes to the
  observation log on the transition into error (status records only the neutral
  `observed.errored` marker), so `GetServer` re-reads the single server by ID when
  the listed status is `ERROR`. Nova's own `_fault_statuses` also covers `DELETED`,
  but the fault is only logged for `ERROR`, so re-reading a deleted server would
  fetch a fault only to discard it.
  ...
  the neutral `errored` marker when Nova reports `ERROR`, logging the fault detail
  on the transition. The marker is gated on
