...
func (c *ComputeClient) RebuildServer(ctx context.Context, id string, imageID regionids.ImageID) (*servers.Server, error) {
	spanAttributes := trace.WithAttributes(
		attribute.String("compute.server.id", id),
		attribute.String("compute.server.action", "rebuild"),
		attribute.String("compute.image.id", imageID.String()),
	)
