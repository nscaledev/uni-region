...
Create recovery is bounded delete-and-retry driven by `ProviderCreateFailures`,
scoped to servers that have never been successfully provisioned (see
Caveats). It leaves the create retry counter and predicate code untouched.

Settlement for create-failure recovery is watch-predicate-driven
(`pkg/managers/server`), over a helper exported from this package: create
failure rides `providerCreateFailureUpdate` over the `ProviderCreateFailure`
helper — it wakes the reconciler to run the bounded delete-and-retry. This
helper is genuinely shared: the provisioner makes the delete-and-retry
decision through the same function the watch predicate fires on, so the
trigger and the action cannot drift.

The image rebuild recovery this package previously also drove — the
arm/submit pass, its `Status.Rebuild` marker, and the `RebuildSettled` /
`serverRebuildSettledUpdate` watch-predicate wake — is excised pending
